...
void DBG::setup (void) {
  #ifdef DEBUG_USART_MODULE_PORTMUX
  PORTMUX.USARTROUTEA |= DEBUG_USART_MODULE_PORTMUX;
  #endif

  #ifdef DEBUG_USART_RX_PIN
  PIN_CTRL(DEBUG_USART_USE_PORT,DEBUG_USART_RX_PIN) = PORT_PULLUPEN_bm | PORT_ISC_INTDISABLE_gc;
  #endif
  DEBUG_USART_USE_PORT.DIRSET = _BV(DBGTX_PIN);
